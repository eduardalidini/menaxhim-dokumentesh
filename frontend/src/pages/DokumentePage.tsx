...
export default function DokumentePage() {
  ...
  async function doArchive() {
    ...
    try {
      const updated = await apiFetch(`/api/documents/${selected.id}/archive`, {
        method: 'PATCH',
      })
      ...
      const msg = e?.payload?.error?.message || 'Gabim gjatë arkivimit'
      setError(msg)
    } finally {
    ...
    <div>
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-1 text-sm text-slate-600">Lista e dokumenteve (aktive/arkivuara)</div>
        </div>
      </div>
      ...
      <ConfirmDialog
        ...
        description={selected ? `"${selected.title}" do të shënohet si archived.` : undefined}
        confirmText="Arkivo"
        onConfirm={doArchive}
        onClose={() => {
